...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const reranker = await prisma.rerankerConfig.update({ where: { id }, data: await request.json() });
    return Response.json(reranker);
  } catch (error) {
    console.error("Error updating reranker:", error);
    return Response.json({ error: "Failed to update reranker" }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!authCheck.authorized) return Response.json({ error: authCheck.error }, { status: authCheck.error === "Unauthorized" ? 401 : 403 });
    const { id } = await params;
    await prisma.rerankerConfig.delete({ where: { id } });
    return Response.json({ success: true });
  } catch (error) {
    console.error("Error deleting reranker:", error);
    return Response.json({ error: "Failed to delete reranker" }, { status: 500 });
  }
}
